...
`{{BASE}}` mirrors the official MCP registry, filtered to the servers an agent
can actually reach.

## Search

```
GET {{BASE}}/api/mcp/search?q=github&auth=none
```
...
Full catalog: `{{BASE}}/api/mcp/catalog.json`.
Aggregates and the exact scope rule: `{{BASE}}/api/mcp/stats.json`.
...
`{{BASE}}/api/mcp/health.json`. A server confirmed unreachable twice running
...
```
claude mcp add --transport http ai-product-index {{BASE}}/mcp
```
