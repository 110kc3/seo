...
`{{BASE}}` mirrors the Coinbase CDP x402 Bazaar and makes it searchable.
...
```
GET {{BASE}}/api/x402/search?q=weather&max_price=0.01
```
...
Full catalog: `{{BASE}}/api/x402/catalog.json`.
Aggregates — price percentiles, chains, host concentration:
`{{BASE}}/api/x402/stats.json`.
...
and results published at `{{BASE}}/api/x402/health.json`; a result confirmed
