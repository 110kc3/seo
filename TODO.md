...
- [ ] **Add Bot Management to the Cloudflare token, then run `gh workflow run cf-admin -f action=robots-allow`.** The automation is written and pushed; it is one permission short. `cf-admin` gained `robots-report` (read-only) and `robots-allow` (writes), which enumerate every zone on the account and clear the managed AI-crawler block. `GET /zones` succeeds — all four zones are listed — but `GET /zones/<id>/bot_management` answers `{"success":false,"errors":[{"code":10000,"message":"Authentication error"}]}` on every one of them, which is Cloudflare's way of saying the token lacks that resource rather than that the setting is absent. Fix: dash.cloudflare.com → **My Profile → API Tokens** → edit the token behind `CLOUDFLARE_API_TOKEN` → add **Zone → Bot Management → Edit** (it currently has Workers/zone-read scopes only) → save. Then run `robots-report` to confirm it reads, and `robots-allow` to apply. The apply prefers `cf_robots_variant: policy_only`, which keeps the Content Signals Policy — the Article 4 reservation — while dropping the `Disallow` rules, and falls back to `is_robots_txt_managed: false` if a zone's plan refuses the variant.
- [ ] **Or turn off the Cloudflare AI-crawler block by hand on three zones.** The only check the fleet still fails anywhere, and the only one no commit can reach: `stareaparaty.com`, `protocolindex.eu` and `overtimelog.com` are each stuck at 90/100 by it. All three serve a `robots.txt` with a Cloudflare-managed block (`# BEGIN Cloudflare Managed content`) disallowing ClaudeBot, GPTBot, CCBot, Google-Extended, Applebot-Extended, Bytespider, Amazonbot and meta-externalagent — injected at the edge *above* each repo's own `Allow: /`, which is why editing the file changes nothing. Cloudflare dashboard → each zone → AI Crawl Control / "Block AI bots". Worth 10 points each. Worth a moment's thought rather than a reflex: on `protocolindex.eu` the block includes **Amazonbot**, on a site whose entire purpose is Amazon affiliate links. If the blocking is deliberate, the lower grades were the honest ones and these should stay as they are.
- [ ] **Re-home the `agent-readability-service` listing onto an origin we control.** It points at `https://github.com/110kc3/seo`, so its score badge reads **E 51** — the Track A shopfront failing its own audit (found in the 2026-07-29 review). Publish a service page on an owned origin (`kc-it.pl` scores A 100; a `/services/agent-readability.html` there is the natural home), then `[update]` the listing URL. The next weekly score cron picks up the new grade.
- [ ] **Directory submissions — the two PR-shaped ones are out; the rest need a browser.** Executed 2026-07-29 with your go: **PR [punkpeye/awesome-mcp-servers#11152](https://github.com/punkpeye/awesome-mcp-servers/pull/11152)** (90k★, the big one, agent-PR fast-track title, upstream CI green) and **PR [SecretiveShell/Awesome-llms-txt#114](https://github.com/SecretiveShell/Awesome-llms-txt/pull/114)**. Both are one-line diffs carrying `https://index.kc-it.pl` and nothing else; both now wait on their maintainers. **agentswelcome.dev refused the submit (HTTP 422)** — its gate is 70/100 and this site scores 60, so that one is a work item, not a waiting item (breakdown and the four fixes in `docs/distribution.md` §4; publishing `/.well-known/agents.json` alone is worth 22 points and clears the gate). **wong2/awesome-mcp-servers no longer takes PRs** — its README redirects submissions to a web form at mcpservers.org/submit. Still manual, all browser sign-ins or forms: llms-txt-hub (llmstxthub.com/submit, GitHub OAuth), directory.llmstxt.cloud (Tally form), llmstxt.site, mcpservers.org. Never submit the old `110kc3.github.io/seo/` URL or the workers.dev fallback.
