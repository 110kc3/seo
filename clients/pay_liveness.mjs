...
//   EVM_PRIVATE_KEY=0x... node clients/pay_liveness.mjs [liveness|route|both] [probe-target]
//
...
if (mode === 'liveness' || mode === 'both') {
  const url = `${BASE}/api/liveness?url=${encodeURIComponent(target)}`;
...
if (mode === 'route' || mode === 'both') {
  const response = await fetchWithPayment(`${BASE}/api/route`, {
    method: 'POST',
